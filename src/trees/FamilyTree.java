...
public class FamilyTree
{
	...
	private void addLine(String line) throws TreeException
	{
		...
		if (colonIndex < 0)
			 throw new TreeException("colonIndex <0 "); //a TreeException with a useful message
		...
		else
		{
			...
			if(parentNode==null)
				throw new TreeException("no parent found");//There's a method in Node that searches for a named node. 
			//??? If the parent node wasn't found, there must have been something wrong in the 
				//data file. Throw an exception.
		}
		for(int p=0; p< childrenArray.length; p++) {
			parentNode.addChild(new TreeNode(childrenArray[p]));
		}
	...
	TreeNode getMostRecentCommonAncestor(String name1, String name2) throws TreeException
	{
		...
		if (node1 == null)
			throw new TreeException("name 1 invalid");//??? Throw a TreeException with a useful message
